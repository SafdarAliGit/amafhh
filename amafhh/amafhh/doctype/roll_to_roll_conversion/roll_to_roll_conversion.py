...
class RollToRollConversion(Document):
    ...
    def on_submit(self):
        ...
        for item in self.roll_to_roll_conversion_target:
            ...
            product_item.gsm = item.gsm if item.gsm else 0
            product_item.width = item.width if item.width else 0
            product_item.length = item.length if item.length else 0
            product_item.stock_uom = 'Kg'
            product_item.is_stock_item = 1
            product_item.rate = item.rate if item.rate else 0
            product_item.amount = item.amount if item.amount else 0
            product_item.qty = item.weight_target if item.weight_target else 0
    ...
    def on_cancel(self):
        stock_entries = get_doctypes_by_field('Stock Entry', 'roll_to_roll_conversion', self.name)
